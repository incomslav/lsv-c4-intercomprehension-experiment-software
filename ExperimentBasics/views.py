...
from LanguageGameExperiment.models import *
...
class UserExperimentOverviewView(View):
    def get(self, request):
        try:
            if UserInfo.objects.filter(user=request.user).exists():
                ...
                try:
                    for i in range(len(exp_with_percentage)):
                        e = exp_with_percentage[i]
                        ...
                        for list in retry_list:
                            ...
                            #    acc += '% (Audio)'
                            if not is_audio:
                                if list.retry_count + 1 > non_audio_max_attempt:
                                    non_audio_max_attempt = list.retry_count + 1

                            temp_dict = {'Correct' : corr, 'Accuracy' : acc, 'Audio' : is_audio}
                        ...
                        # AYAN
                        # IF NOT AUDIO RETRY... WE MUST MAKE SEPARATE PLOTS FOR TEXT AND AUDIO LATER!
                        if non_audio_max_attempt > -1:
                            max_retry_score = float(retry_dict[non_audio_max_attempt]['Accuracy'])
                            # UPDATE!
                            list_plot[i][1] = max_retry_score
                ...
                try:
                    ...
                    return HttpResponse('Problem in 2nd')

                # for Language Game Experiments
                try:
                    cleared_for_lang_game = False
                    user_languages = userInfo.getAllLanguages()
                    allowed_languages = ['cs', 'ru', 'bg', 'pl']

                    for lang in user_languages:

                        if lang.language_code in allowed_languages:
                            cleared_for_lang_game = True



                    lado_available_experiments = []
                    if cleared_for_lang_game:
                        lado_experiments = LaDoExperiment.objects.filter(is_active=True).order_by('-id')
                        for exp in lado_experiments:
                            if exp .experiment_type == 'de compound':
                                if not DECompoundExperimentParticipation.objects.filter(user=userInfo, experiment=exp, completed_on__isnull=False).exists() and userInfo.clearedForDECompoundRUContextExperiment():
                                    lado_available_experiments.append(exp)

                            elif exp.experiment_type == 'sentence translation':
                                native_lang = userInfo.getNativeLanguages()
                                native_lang_code = []
                                for nl in native_lang:
                                    native_lang_code.append(nl.language_code)

                                if exp.experiment_language in native_lang_code:
                                    lado_available_experiments.append(exp)

                            else:
                                if not LaDoExperimentParticipation.objects.filter(user=userInfo, experiment=exp, completed_on__isnull=False).exists() and not PrimalExperimentParticipation.objects.filter(user=userInfo, experiment=exp, completed_on__isnull=False).exists():
                                    lado_available_experiments.append(exp)


                    print(lado_available_experiments)
                except:
                    HttpResponse('Problem in 3')

                # AYAN
                # try:
                #     user_native_lang = userInfo.getNativeLanguages()
                #     close_test_experiments = CloseTestExperiment.objects.filter()
                ...
                return render(request, "ExperimentOverview.html", {'completed_experiments': exp_with_percentage,
                                                                   ...
                                                                   'lado_available_experiments': lado_available_experiments})
    ...
    def post(self, request):
        ...
        # new experiment , another new block Language Game

        if 'lado_type' in request.POST:



            try:
                exp_id = int(str(request.POST["hdnId"]).strip())
                lado_exp = LaDoExperiment.objects.get(id=exp_id)
                print('Selected Lado ' + str(exp_id))
                exp_type = request.POST['lado_type']
                print("exp type", exp_type)

                if exp_type == 'primal':
                    request.session['PRIMAL_EXP_ID'] = lado_exp.id
                    return HttpResponseRedirect('/LanguageGameExperiment/PrimalTaskExperimentText')

                elif exp_type == 'de compound':
                    return HttpResponseRedirect('/LanguageGameExperiment/12/DECompoundWelcomePage/')

                elif exp_type == 'sentence translation':
                    exp_link = '/LanguageGameExperiment/' + str(exp_id) + '/SentenceTranslationWelcomePage/'
                    return HttpResponseRedirect(exp_link)

                else:
                    request.session['LADO_SELECTED_EXP'] = lado_exp.id
                    print(request.session['LADO_SELECTED_EXP'])


                    return HttpResponseRedirect('/LanguageGameExperiment/ExperimentTextView')
        ...
        if 'status' in request.POST:
            try:
                ...
                request.session['EXPERIMENT_STATUS'] = 'complete'
                request.session[RETRY_EXPERIMENT_STATUS] = request.POST['status']
                print(request.session['EXPERIMENT_STATUS'], request.session[RETRY_EXPERIMENT_STATUS])
                ...
                request.session[RETRY_ASSIGNED_EXPERIMENT] = exp_id
                print("Retry exp id: ", request.session[RETRY_ASSIGNED_EXPERIMENT])
    ...
    def getExperimentWithMedals(userInfo):
        ...
        for ep in ExperimentParticipation.objects.filter(user=userInfo, completed_on__isnull=False).distinct(
            ...
            if "Gap Filling" in ex.getExperimentNameForUser():
                num_total, num_correct, num_incorrect, num_not_answered, total_time_ms, total_gaps, correct_gaps, incorrect_gaps = ep.getResultsForGapFilling()
            else:
                num_total, num_correct, num_incorrect, num_not_answered, total_time = ep.getResults()
            logger.error("!!!!! getExpWithMedals "+str(num_total)+" "+str(num_correct)+" "+str(num_incorrect)+" "+str(num_not_answered))
...
class RetrySelectExperimentView(View):
    ...
    def get(self, request):
        ...
        try:
            ...
            if RETRY_ASSIGNED_EXPERIMENT in request.session:
                ...
                # if SELECTED_EXPERIMENT_TYPE in request.session:
                #     print("SelectExperiment ASSIGNED_EXPERIMENT True Audio:", request.session[SELECTED_EXPERIMENT_TYPE])
                #     exper_type = request.session[SELECTED_EXPERIMENT_TYPE];
                # else:
                #     exper_type = 'text';

                #
                # if RETRY_AUDIO_EXPERIMENT in request.session or request.session[RETRY_AUDIO_EXPERIMENT] == 'audio':
                #     exper_type = 'audio'
                # else:
                #     exper_type = 'text'
                ...
                # for k, v in request.session.items():
                #     print(k, ":", v)
                demo_audio_files = ['/static/media/audio_experiment.mp3', '/static/media/Konferenz.mp3',\
                 '/static/media/Situation.mp3']
                demo_answers = ['audio experiment', 'conference', 'situation']
                ...
                #                                                   'experiment_type': exper_type,
                #                                                   'demo_audio_files': demo_audio_files,
    ...
    def post(self, request):
        ...
        try:
            ...
            print("Printing k in post retry")
            for k, v in request.session.items():
                print(k, ":", v)

            experiments = Experiment.objects.filter(id=request.session[RETRY_ASSIGNED_EXPERIMENT]).select_subclasses()
            print(request.session[RETRY_ASSIGNED_EXPERIMENT], "in post ")
            if len(experiments) > 0:
...
class SelectExperimentView(View):
    ...
    def get(self, request):
        ...
        try:
            ...
            if ASSIGNED_EXPERIMENT in request.session:
                ...
                # for k, v in request.session.items():
                #     print(k, ":", v)
                demo_audio_files = ['/static/media/audio_experiment.mp3', '/static/media/Konferenz.mp3', '/static/media/Situation.mp3']
                demo_answers = ['audio experiment', 'conference', 'situation']
                return render(request, 'ExperimentWelcome.html', {'strings': selected.getWelcomeTemplateStrings(),
                                                                  'ExperimentWelcomeScreenConstants': ExperimentWelcomeScreenConstants,
                                                                  'enableJavaScriptMessage': EnableJavaScriptMessageConstants,
            ...
            if preselected:
                ...
                print("SelectExperiment preselected")
                ...
                if selected.text_experiment:
                    request.session[SELECTED_EXPERIMENT_TYPE] = 'text'
                elif selected.audio_experiment:
                    request.session[SELECTED_EXPERIMENT_TYPE] = 'audio'
                else:
                    request.session[SELECTED_EXPERIMENT_TYPE] = 'text'
    ...
    def getAvailableExperiments(self,experiments, userInfo, userNativeLanguages, userAllLanguages, isEnglish):
        ...
        for experiment in experiments:
            ...
            if experiment.foreign_script is not None:
                ...
                foreign_lang_script = LanguageScriptRelation.objects.filter(language=foreign_lang)
                continue_flag = True
                logger.error("******** GetAvailableExp: Exp foreign lang script is None... For_Lang:"+str(foreign_lang)+" Nat_Lang:"+str(native_language)+" **********")
            ...
            if not isEnglish:
                logger.error("### GetAvailableExp: notisEnglish... "+str(foreign_lang)+" "+str(native_language))
                if experiment.userFulfillsPrerequisites(userInfo):
                    logger.error("%%% Satisfies Prereqs! "+str(last_priority))
                ...
                exp, last_priority = self.getNotCompletedExperiment(experiment, userInfo,
                                                                                   ...
                                                                                   userAllLanguages, last_priority)
            if exp is not None:
                logger.error("GetAvailableExp: Append exp")
    ...
    def getNotCompletedExperiment(self,experiment, userInfo, userNativeLanguages, userAllLanguages, lastPriority):
        ...
        if not experiment.userHasCompletedExperiment(userInfo):
            logger.error("@@@ getNotCompletedExp: not user has completed...")
            keep = False
...
class InspectExperimentsGridView(BaseDatatableView):
    ...
    columns = ['experiment_name', 'is_active', 'text_experiment', 'audio_experiment', 'priority', 'user_prerequisites', 'freetranslationexperiment__foreign_language__language_name','number_of_questions',
               'number of answers', 'number of answers audio', 'created_on', 'details', 'users', 'stimuli_file', 'folder_name', 'is_audio_experiment']
    ...
    def render_column(self, item, column):
        ...
        if column == 'is_active':
        ...
        elif column == 'number of answers audio':
            ranks = Experiment.objects.get(pk=item.id).getNumbersOfCollectedAnswersForAudio()
            out = []
            for rank, number in sorted(ranks.items(), key=lambda x: x[0]):
                out.append(str(number) + "@" + str(rank + 1))
            if out:
            ...
            if "Gap Filling" in experimentName:
                return 'download answers (<a onclick="redirectTo(\'admin/ExportUserAnswersCSV/' + str(
                    ...
                    item.id) + '/\');">xlsx</a>)'
            if experiment.is_audio_experiment:
                return 'download answers (<a onclick="redirectTo(\'admin/ExportUserAnswersCSV/' + str(
                item.id) + '/\');">csv</a> | <a onclick="redirectTo(\'admin/ExportUserAnswersXLSX/' + str(
                item.id) + '/\');">xlsx</a> | <a onclick="redirectTo(\'admin/ExportAudioUserAnswersCSV/' + str(
                item.id) + '/\');">audio_csv<a/>)'
            else:    
                return 'download answers (<a onclick="redirectTo(\'admin/ExportUserAnswersCSV/' + str(
                item.id) + '/\');">csv</a> | <a onclick="redirectTo(\'admin/ExportUserAnswersXLSX/' + str(
