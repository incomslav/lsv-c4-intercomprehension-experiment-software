...
# Not in use
# def changePhraseTranslationExperimentFileActivationStatus(userId, fileId, activationStatus):
#     try:
#         PhraseTranslation_file = PhraseTranslationUploadedExperimentQuestionFile.objects.get(pk=fileId)
#         if activationStatus != PhraseTranslation_file.is_active:
#             PhraseTranslation_file.is_active = activationStatus
#             # set all associated questions to the given activation status
#             for question in PhraseTranslation_file.associated_questions.all():
#                 question.is_active = activationStatus
#                 question.save()
#             PhraseTranslation_file.save()
#             return True
#         return False
...
def processPhraseTranslationHeaders(headers):
    print(headers)
...
def processPhraseTranslationHeadersNew(headers):
    print(headers)
...
def parsePhraseTranslationFileRow(row, headers):
    meta = {}
...
def processPhraseTranslationUploadedFile(location, filename, experiment_name, priority, medal_filename = None):
    ...
    for ws in f.worksheets:
        ...
        if not headers is None:
                # Lnative, Lforeign, headers = processPhraseTranslationHeaders(headers)

                Lnative, Lforeign, Snative, Sforeign, headers = processPhraseTranslationHeadersNew(headers)
                ...
                #Added by Hasan
                # folder_name = os.path.basename(settings.PHRASE_TRANSLATION_UPLOAD_FOLDER)
                folder_name = 'PhraseTranslation'

                for d in data:
                    native_entry, foreign_entry, group, alternatives, meta = parsePhraseTranslationFileRow(d,headers)
                ...
                for group, L in groups.items():
                    ...
                    experiment_object = PhraseTranslationExperiment()
                    ...
                    for native_entry, foreign_entry, alternatives, meta in L:
                        ...
                        # make PhraseTranslationQuestion
                        FQ = PhraseTranslationQuestion(foreign_word=foreign_entry)
                        FQ.save()
                        # make all correct answers
                        correct_answers = []
                        FA = PhraseTranslationCorrectAnswer()
                        FA.native_word = native_entry
                        FA.save()
                        FQ.correct_answers.add(FA)
                        for alternative in alternatives:
                                FAlt = PhraseTranslationCorrectAnswer()
