...
class GapFillingExperiment(Experiment):
    """
    Gap Filling Experiment class
    """
    gap_filling_experiment_id = models.AutoField(primary_key=True)
    native_language = models.ForeignKey(Language, related_name="GapFilling_native_language", null=True)
    foreign_language = models.ForeignKey(Language, related_name="GapFilling_foreign_language", null=True)
    ...
    def getExperimentNameForUser(self):
        ...
        return "Gap Filling " + self.native_language.language_code + "-" + self.foreign_language.language_code

    def getRedirectURL(self):
        """ The URL this experiment links to for its actual questions. """
        return reverse("GapFillingQuestions")
    ...
    def getWelcomeTemplateStrings(self):
        ...
        try:
            ...
            strings['EXPERIMENT_WELCOME_HEADER'] = _("GAP_FILLING_EXPERIMENT_WELCOME_HEADER") + " #" + str(
                self.gap_filling_experiment_id)
            ...
            <p>When you are ready, check 'I am ready' and click on the 'start experiment' button to proceed.<p>""".format(
                str(self.getNumberOfQuestions()), self.foreign_language.language_name,
...
class GapFillingAnswer(models.Model):
    """ Model for any answer in Gap Filling task.
    Stores the comma separated gaps in the gaps. """
    gap_filling_answer_id = models.AutoField(primary_key=True)
...
class GapFillingQuestion(Question):
    ...
    acceptable answers. """
    gap_filling_question_id = models.AutoField(primary_key=True)
    ...
    correct_answers = models.ManyToManyField(GapFillingCorrectAnswer, related_name="associated_question")
...
class GapFillingUserAnswer(UserAnswer, GapFillingAnswer):
    """ A Gap Filling user answer. Stores the final reply and a
    JSON-encoded log of the gaps changes and (millisecond)
    times of change."""
    gap_filling_user_answer_id = models.AutoField(primary_key=True)
        ...
        try:
            if self.normalized_form_is_correct is None and not self.gaps_answers is None:
                ...
                for ca in correct_answers:
                    k, val = ca.split('_')
                    # Multiple space tolerance
                    val = " ".join(val.strip().lower().split())
                    if k not in ca_dict:
                ...
                for i in range(len(answers)):
                    # Multiple space tolerance
                    answers[i] = " ".join(answers[i].lower().strip().split())
                ...
                # return True
                logger.error("&&& Returning Dict..." + str(userAnsCorrectnessDict))
