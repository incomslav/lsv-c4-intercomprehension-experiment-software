...
from GapFillingExperiment.importer import *
from GapFillingExperiment.models import *
from GapFillingExperiment.forms import *
...
class GapFillingExperimentsGridView(BaseDatatableView):
    """
    Grid view for uploaded files record
    """
    model = GapFillingExperiment

    # define the columns that will be returned
    columns = ['gap_filling_experiment_id', 'experiment_name', 'native_language.language_code',
               'foreign_language.language_code', 'is_active', 'priority', 'number_of_questions', 'num_answers',
               'created_on', 'delete']
    order_columns = ['gap_filling_experiment_id', 'experiment_name', 'native_language.language_code',
                     'foreign_language.language_code', 'is_active', 'priority', 'number_of_questions', 'num_answers',
                     'created_on', 'delete']
    ...
    def get_initial_queryset(self):
        ...
        # for a in ls:
        #     a.delete()
        return GapFillingExperiment.objects.all()

    def render_column(self, item, column):
        if column == 'delete':
            return "<a onclick='deleteExperiment({});' >delete</a>".format(item.gap_filling_experiment_id)
        ...
        if column == 'is_active':
            if item.is_active:
                return '<input id="is_active_input_' + str(
                    ...
                    item.gap_filling_experiment_id) + "checked >"
            else:
                return '<input id="is_active_input_' + str(
                    item.id) + ';" type="checkbox" onclick="changeActivationStatus({}, false)" '.format(
                    item.gap_filling_experiment_id) + " >"
            ...
            return super(GapFillingExperimentsGridView, self).render_column(item, column)
...
class UploadGapFillingExperimentFile(View):
    """
    ...
    def get(self, request):
        ...
        try:
            return render(request, 'adminpanel/UploadGapFillingExperimentFile.html',
        ...
        if request.is_ajax():
            try:
                ...
                if requestType == '1':
                    ...
                    E = GapFillingExperiment.objects.get(pk=experiment_id)
                    E.is_active = True
                    E.save()
                ...
                # delete all experiments
                    ...
                    E = GapFillingExperiment.objects.all()
                    for e in E:
                        e.delete()
                    return HttpResponse(True)
...
class GapFillingQuestionsView(View):
        ...
        try:
            ...
            if ASSIGNED_EXPERIMENT in request.session:
                if CURRENT_PARTICIPATION in request.session:
                    ...
                    return render(request, 'GapFillingNewVersion.html', params)
            ...
            if request.is_ajax():
                ...
                if requestType == '1':
                    ...
                    if lastAnswerId != '':
                        ...
                        answer_object = GapFillingUserAnswer.objects.get(useranswer_ptr_id=lastAnswerId)
                    ...
                    if nextID is None:
                        ...
                        nextAnswer = GapFillingUserAnswer.objects.get(useranswer_ptr_id=nextID)
                        nextQuestion = GapFillingQuestion.objects.get(question_ptr_id=nextAnswer.answered_question.id)
